...
@QuarkusTest
@TestProfile(KmsCreateAliasAuthorizationIntegrationTest.IamEnforcementProfile.class)
class KmsCreateAliasAuthorizationIntegrationTest {
    ...
    @Test
    void authorizesBothAliasNamespaceAndTaggedTargetKey() {
        KeyIdentity matchingKey = createKey(REQUIRED_TAGS);
        KeyIdentity wrongTagKey = createKey(Map.of(
                "example.io:definition-id", "other",
                "example.io:managed-by", "floci"));
        SessionCredentials credentials = callerWithCreateAliasPolicy();
        String suffix = UUID.randomUUID().toString().substring(0, 8);

        createAlias(credentials, "alias/approved/credentials-" + suffix, matchingKey.keyId())
                .statusCode(200);

        createAlias(credentials, "alias/other/credentials-" + suffix, matchingKey.keyId())
                .statusCode(403)
                .body("__type", equalTo("AccessDeniedException"));
        createAlias(credentials, "alias/approved/wrong-tag-" + suffix, wrongTagKey.keyId())
                .statusCode(403)
                .body("__type", equalTo("AccessDeniedException"));
    }
    ...
    private static KeyIdentity createKey(Map<String, String> tags) {
        var response = given()
                ...
                .body(Map.of(
                        "Description", "KMS CreateAlias authorization integration test",
                        "Tags", tagList(tags)))
        .when()
                .post("/")
        .then()
    ...
    private static SessionCredentials callerWithCreateAliasPolicy() {
        String suffix = UUID.randomUUID().toString().substring(0, 8);
        String roleName = "KmsAliasCreator" + suffix;
        createRole(roleName);
        putRolePolicy(roleName, "KmsCreateAliasAuthorization", """
                {
                  "Version": "2012-10-17",
                  "Statement": [
                    {
                      ...
                      "Resource": "arn:aws:kms:%s:%s:alias/approved/*"
                    },
                    {
                      "Effect": "Allow",
                      "Action": "kms:CreateAlias",
                ...
                """.formatted(REGION, ACCOUNT_ID, REGION, ACCOUNT_ID));
    ...
    private static SessionCredentials assumeRole(String roleName) {
        var response = given()
                ...
                .formParam("RoleSessionName", "kms-create-alias-authorization-test")
    ...
    private static io.restassured.response.ValidatableResponse createAlias(
            SessionCredentials credentials, String aliasName, String targetKeyId) {
        return given()
                ...
                .contentType(KMS_CONTENT_TYPE)
                .body(Map.of("AliasName", aliasName, "TargetKeyId", targetKeyId))
        .when()
                .post("/")
        .then();
    }
