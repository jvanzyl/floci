...
public class ScheduleInvoker {
    ...
    private void deliverToEcsRunTask(Target target, String region) {
        ...
        String taskDefinitionArn = ecs.getTaskDefinitionArn();
        if (taskDefinitionArn == null || taskDefinitionArn.isBlank()) {
            LOG.warnv("Scheduler: ECS target {0} is missing TaskDefinitionArn", target.getArn());
            return;
        }
        ecsService.runTask(
                target.getArn(),
                taskDefinitionArn,
                ecs.getTaskCount() != null && ecs.getTaskCount() > 0 ? ecs.getTaskCount() : 1,
